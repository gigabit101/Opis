...
public class PacketManager {
    ...
    public static void splitAndSend(Message msg, List<? extends ISerializable> data, EntityPlayerMP player) {
        int i = 0;
        while (i < data.size()) {
    ...
    public static void sendFullUpdate(EntityPlayerMP player) {
        ...
        List<StatsChunk> timingChunks = ChunkManager.INSTANCE.getTopChunks(100);
        List<DataEntityPerClass> timingEntsClass = EntityManager.INSTANCE.getTotalPerClass();
        List<DataBlockTileEntityPerClass> timingTEsClass = TileEntityManager.INSTANCE.getCumulativeTimingTileEntities();
        ...
        PacketManager.validateAndSend(new NetDataList(Message.LIST_TIMING_HANDLERS, Profilers.ASM_EVENT_HANDLER.get().getTickData()), player);
        ...
        PacketManager.validateAndSend(new NetDataList(Message.LIST_TIMING_EVENTS, Profilers.ASM_EVENT_HANDLER.get().getData()), player);
